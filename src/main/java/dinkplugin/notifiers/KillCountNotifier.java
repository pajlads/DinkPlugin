...
public class KillCountNotifier extends BaseNotifier {
    ...
    private static final Pattern PRIMARY_REGEX = Pattern.compile("Your (?<key>.+)\\s(?<type>kill|chest|completion|harvest|success|opened)\\s?count is: ?(?:@mes_hl_red@)?(?<value>[\\d,]+)\\b", Pattern.CASE_INSENSITIVE);
